...
use crate::units::{Hertz, Meters, PerCubicMeter};
use thiserror::Error;
...
/// Chapman layer: Ne = Nm exp(1/2 (1 - z - e^{-z})), z = (r - r_m)/H.
/// The classic profile of a monochromatically-absorbed ionising flux in an
/// exponential atmosphere, taken at overhead sun (sec chi = 1); smooth
/// everywhere (C-infinity), never exactly zero - callers wanting vacuum
/// below must rely on its exponential decay, which step control handles.
...
impl ElectronDensity for ChapmanLayer {
    fn sample(&self, p: &SphericalPoint) -> DensitySample {
        ...
        let ne = self.nm * (0.5 * (1.0 - z - (-z).exp())).exp();
        // dNe/dr = Ne * (1/2)(-1 + e^{-z}) / H.
        let dne = ne * 0.5 * ((-z).exp() - 1.0) / self.scale_height;
        DensitySample {
            ne,
            d_ne: [dne, 0.0, 0.0],
        }
